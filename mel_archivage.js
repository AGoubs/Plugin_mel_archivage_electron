                ...
                rcmail.addEventListener('responseafterplugin.mel_archivage_traitement_electron', function (event) {
                    ...
                    for (const mbox in parsedObj) {
                        for (let i = 0; i < parsedObj[mbox].length; i++) {
                            ...
                            files.push({ "url": rcmail.url('mail/viewsource', rcmail.params_from_uid(uid.message_uid)).replace(/_framed=/, '_save='), "uid": uid.message_uid, "path_folder": rcmail.env.account + "/" + mbox, "mbox": mbox, "etiquettes": uid.flags });
                        }
